...
df = pd.read_csv('/Users/algaumon/Documents/Projet_2/VS_code_akas/df_v3.csv', index_col=0)
df_actor_pop = pd.read_csv('/Users/algaumon/Documents/Projet_2/VS_code_akas/df_actor_actress_updated.csv',index_col=0)
df_director_pop = pd.read_csv('/Users/algaumon/Documents/Projet_2/VS_code_akas/director_popularities.csv',index_col=0)
...
def mark_as_seen(movie):
    # Initialisation de la liste si elle n'existe pas
    if 'viewed_movies' not in st.session_state:
        st.session_state.viewed_movies = []
    
    # Ajouter ou retirer le film de la liste des films vus
    if movie not in st.session_state.viewed_movies:
        st.session_state.viewed_movies.append(movie)
        st.write(f"Vous avez marqué '{movie['Titre']}' comme vu !")
    else:
        st.session_state.viewed_movies.remove(movie)
        st.write(f"Vous avez retiré '{movie['Titre']}' de votre liste de films vus.")
...
if selection == "Accueil":
    ...
    if genre or acteur or real or film:
        ...
        if genre:
            filtered_films = df[df['genres_x'].str.contains(genre, case=False, na=False)] 

        # Appliquer le filtre pour l'acteur si sélectionné
        if acteur:
            filtered_films = df[df['cast'].str.contains(acteur, case=False, na=False)] 

        # Appliquer le filtre pour le réalisateur si sélectionné
        if real:
            filtered_films = df[df['director'].str.contains(real, case=False, na=False)]

        # Appliquer le filtre pour le film si sélectionné
        if film:
            filtered_films = df[df['title_y'].str.contains(film, case=False, na=False)]

        # Trier les films par moyenne de note et sélectionner les 3 premiers
        films = df.sort_values(by='averageRating', ascending=False).head(3)
        
        # Afficher les suggestions de films
        st.subheader("Suggestions de films")

        # Si des films sont filtrés, afficher les suggestions
        if not films.empty:
            for index, row in films.iterrows():
                col1, col2 = st.columns([1, 2])
                with col1:
                    st.image(row['poster_path'], caption=row['title_y']) 

                with col2:
                    # Conversion et affichage des vidéos 
                    try:
                        st.video(row['key'])
                    except Exception as e:
                        st.write("Video Indisponible")  # Afficher un message d'erreur spécifique pour la vidéo

                st.write(f"**Acteur** : {row['cast']}")
                st.write(f"**Nom du film** : {row['title_y']}") 
                st.write(f"**Synopsis** : {row['overview']}")
                st.markdown("<hr style='border: 2px solid #f08080;'>", unsafe_allow_html=True)
        else:
            st.write("Aucune suggestion de film ne correspond aux critères.")
    else:
        st.write("Veuillez remplir au moins un critère pour obtenir des recommandations.")
    ...
    # Contenu de la page d'accueil
                ...
                st.subheader("Choisissez un genre pour decouvrir les meilleurs films du genre")
                ...
                genre = st.segmented_control(label="Choisissez un genre", options=genres)
# Filtrer le DataFrame pour le genre sélectionné
                if genre:
                        ...
                        if top_10_films.empty:
                            ...
                            for index, row in top_10_films.iterrows():
                                ...
                                with col1:
                                    st.subheader(f"**{row['title_y']}** ({row['startYear']})")
                                with col2:
                                     if st.button(f"J'ai vu", key=f"btn_{row['title_y']}"):
                                        mark_as_seen(row)
                            
                                col1, col2 = st.columns([1, 2])
                                with col1:
                                    # Affichage de l'image du film
                                    st.image(row['poster_path'])  # Lien de l'image
                                with col2:
                                    try:
                                        if row['key']:
                                            st.video(row['key'])
    ...
    if st.session_state.viewed_movies:
        ...
        # Parcours de chaque film dans la liste
        for movie in st.session_state.viewed_movies:
            st.subheader(movie['Titre'])
            ...
            with col1:
                ...
                if movie['Affiche']:  # Vérification si l'URL de l'affiche existe
                    st.image(movie['Affiche'], caption=movie['Titre'], use_container_width=True)
